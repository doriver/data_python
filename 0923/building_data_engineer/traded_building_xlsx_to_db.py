# 일반건물(상업업무용/공장창고등) 실거래가 매핑 xlsx를 읽어 MySQL에 적재하는 스크립트
#
...
#
# data/processed 안의 "*_좌표.xlsx"를 훑되, 같은 디렉터리에는 토지/단독다가구용
# _좌표.xlsx도 함께 생성될 수 있으므로 헤더의 지번/건축물주용도 컬럼 유무로
# 일반건물 실거래 스키마인 파일만 골라 처리한다 (find_general_building.py의
# _is_general_building_file과 동일한 판별 방식).
#
# PNU가 채워진(매칭 성공) 행만 저장한다. (pnu, 연면적, 대장pk) 조합이 처음
# 등장할 때만 traded_general_building/general_building_mapping_building에
# insert하고, 같은 조합이 다시 나오면 traded_general_building_detail에만
# insert한다 (거래 1건당 1행). 건축년도는 결측치 비중이 높아(약 10.8%) dedup 키에서 제외했다 (포함 시 같은 건물이 건축년도 결측 여부만으로 쪼개지는 사례 존재).
#
# 총괄표제 존재여부가 O인 경우 대장pk가 콤마로 구분된 여러 값일 수 있으며, 이때
# general_building_mapping_building에는 값마다 개별 행으로 저장한다. 대장pk가
# 없는 행은 general_building_mapping_building에 매핑 행을 만들지 않는다.
import glob
import math
import os
import sys
...
REQUIRED_COLUMNS = {"지번", "건축물주용도"}  # 일반건물 실거래 스키마 판별용
BUILDING_TABLE_NAME = "traded_general_building"
DETAIL_TABLE_NAME = "traded_general_building_detail"
MAPPING_TABLE_NAME = "general_building_mapping_building"
...
CREATE TABLE IF NOT EXISTS {DETAIL_TABLE_NAME} (
    id BIGINT NOT NULL AUTO_INCREMENT,
    traded_general_building_id BIGINT NOT NULL,
    pnu VARCHAR(20) NOT NULL,
    ...
    PRIMARY KEY (id),
    INDEX idx_traded_general_building_id (traded_general_building_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
"""
...
INSERT_BUILDING_SQL = f"""
INSERT INTO {BUILDING_TABLE_NAME}
    (pnu, sigungu, building_type, beonji, road_name,
     use_district, main_use, road_condition, total_floor_area, site_area,
     built_year, confirmed_beonji, has_overall_title, land_area, official_land_price,
     coordinates)
VALUES
    (%s, %s, %s, %s, %s,
     %s, %s, %s, %s, %s,
...
INSERT INTO {DETAIL_TABLE_NAME}
    (traded_general_building_id, pnu, contract_year_month, contract_day,
...
INSERT INTO {MAPPING_TABLE_NAME}
    (traded_general_building_id, building_pk)
VALUES
    (%s, %s)
"""
...
def is_general_building_file(xlsx_path):
    columns = set(pd.read_excel(xlsx_path, header=HEADER_ROW, nrows=0).columns)
    return REQUIRED_COLUMNS.issubset(columns)


all_xlsx_paths = sorted(glob.glob(os.path.join(XLSX_DIR, XLSX_GLOB)))
xlsx_paths = []
for xlsx_path in all_xlsx_paths:
    if is_general_building_file(xlsx_path):
...
print(
    ...
    f"총 행 수: {total_count}, 고유 (pnu,연면적,대장pk) 수(거래된 건물) 수: {len(building_rows)}, "
...
try:
    with conn.cursor() as cursor:
        cursor.execute(CREATE_BUILDING_TABLE_SQL)
        cursor.execute(CREATE_DETAIL_TABLE_SQL)
        cursor.execute(CREATE_MAPPING_TABLE_SQL)
        conn.commit()

        # 거래된 건물: (pnu,연면적,대장pk) -> traded_general_building.id
        key_to_building_id = {}
        inserted_building = 0
        for i, r in enumerate(building_rows, start=1):
            key, r = r[0], r[1:]
            cursor.execute(INSERT_BUILDING_SQL, r)
            key_to_building_id[key] = cursor.lastrowid
            inserted_building += 1
            if i % INSERT_BATCH_SIZE == 0:
                conn.commit()
                print(f"거래된 건물 삽입 진행: {inserted_building}/{len(building_rows)}")
        conn.commit()
        print(f"거래된 건물 삽입 완료: {inserted_building}/{len(building_rows)}")

        # 건물 매핑: 대장pk가 콤마로 구분된 경우 값마다 개별 행으로 저장한다.
        mapping_params = [
            (key_to_building_id[key], pk)
        ...
        # 거래상세: 위에서 확보한 traded_general_building_id를 채워 넣는다.
        detail_params = [
            (key_to_building_id[d[0]], *d[1:]) for d in detail_rows
        ]
        inserted_detail = 0
    ...
    print(
        f"완료: 거래된 건물 {inserted_building}건, 건물 매핑 {inserted_mapping}건, "
