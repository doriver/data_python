...
DATA_LAKE_DIR = Path(r"D:\tmpData") # r"D:\data\data_lake"
...
class DailyDownloadLimitError(RuntimeError):
    """일일 다운로드 건수 제한(세션/IP당 100건)을 초과했을 때 발생"""
...
def download_excel(
    ...
    *,
    ...
    # Content-disposition이 없으면 파일이 아니라 JSON 에러 응답
    # (예: {"error":"일일 다운로드 횟수는 최대 100건 입니다."})
    if not disposition and resp.content.strip().startswith(b"{"):
        try:
            error_msg = resp.json().get("error", resp.text)
        except ValueError:
            error_msg = resp.text
        if "100건" in error_msg:
            raise DailyDownloadLimitError(error_msg)
...
def main():
    ...
    for item in sido_list:
        ...
        try:
            ...
            print(f"[완료] {renamed}")
        except DailyDownloadLimitError as e:
            print(f"[중단] 일일 다운로드 한도 초과로 나머지 시도는 스킵합니다: {e}")
            break
